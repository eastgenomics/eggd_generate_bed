...
main() {
    ...
    if [[ -n ${optional_args+x} ]]; then
        if [[ -n "$additional_regions" ]]; then
            python3 ~/generate_bed.py -p "$panel" -e ~/"$exons_name" -g ~/"$gene_panels_name" -t ~/"$genes2transcripts_name" -a ~/"$additional_regions_name" -h "$header_info" $optional_args
        else
            python3 ~/generate_bed.py -p "$panel" -e ~/"$exons_name" -g ~/"$gene_panels_name" -t ~/"$genes2transcripts_name" -h "$header_info" $optional_args
        fi
    else
        python3 ~/generate_bed.py -p "$panel" -e ~/"$exons_name" -g ~/"$gene_panels_name" -t ~/"$genes2transcripts_name" -h "$header_info"
    fi
